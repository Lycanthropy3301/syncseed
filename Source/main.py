...
# Declaring the Seed_Value challenge upper and lower bounds. These must be consistent with both the server and client.
SEED_VALUE_LOWER_BOUND = 100_000_000_000
SEED_VALUE_UPPER_BOUND = 999_999_999_999

# Declaring standard server responses.
...
# Dictionary comprehensions to intialise dictionaries to hold username and passwords as well as seed and seed increment values tied to each username.
...
def authenticated(user, password, seed_value):
    ...
    # Checks of password and seed_value use guard clause structure.
    try:

        # Check if user passwords match, return false if they don't.
        ...
        # Upper and Lower bounds for the generated number were declared earlier in the code.
        if seed_value == generator.integers(SEED_VALUE_LOWER_BOUND, SEED_VALUE_UPPER_BOUND):
            return True
...
def handle_connection(client):
    ...
    try:
        ...
        # Call to the authentication function
        if authenticated(username, password, seed_value):
            ...
            # Updates the server seed dictionary, incrementing the seed increment value.
