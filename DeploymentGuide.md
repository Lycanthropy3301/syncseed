...
In this section I will go over what each configuration option for the Syncseed generator does, one-by-one.
1. `set_seed_length(length)`
...
4. `set_seed_value_lower_bound(lower_bound)` and `set_seed_value_upper_bound(upper_bound)`
...
5. `set_cha_cha_generator_rounds(rounds)`
